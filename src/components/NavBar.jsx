...
const NavBar = () => {
  ...
  return (
    <div className='flex justify-between items-center w-full h-20 text-white px-4 fixed bg-black'>
        <div>
            <h1 className='text-5xl font-signature ml-2 text-cyan-300 '>Ayush</h1>
        </div>
        <ul className='hidden md:flex'>
        {links.map(({id,link})=>{
            return <li key={id} className='px-4 capitalize cursor-pointer font-medium text-gray-500 hover:scale-105 duration-200'><Link to={link} smooth duration={500}>{link}</Link></li>
        })}
        </ul>
        <div onClick={()=>{
            setnav(!nav)
